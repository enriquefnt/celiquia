...
class Ficha
{
    ...
    public function constancia()
    {
        ...
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(0, 7, iconv('UTF-8', 'Windows-1252', 'Información requerida para el diagnóstico'), 1, 0, 'C');
        $pdf->Ln(10);
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(100, 10, iconv('UTF-8', 'Windows-1252', 'Forma clínica de presentación: ') . iconv('UTF-8', 'Windows-1252', $datosFicha['formaclin']), 0, 0);
        $pdf->Ln(6);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Arial', '', 10);
        $pdf->Ln(8);
